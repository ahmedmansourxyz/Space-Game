...
int main()
{
	...
	if ( !Inistialiser_Perso( &Perso, "perso.bmp" ) )
	{
        ...
        if ( !Inistialiser_Perso( &Ennemie, "Ennemie.bmp" ) )
	{        Supprimer_Perso( &Ennemie);
	...
	// on lance un deplacement aléatoire pour le Sprite de l'ennemie
	// on genere un nombre entre 1, 2, 4 et 8 pour chaque sprite
	Direction_Perso( &Ennemie , 1 << (rand()%4) );
	...
	while (!continuer)
	{
		...
		if (temps_courant > prochain_rendu)
		{
			...
			miseAJourSprite( &Perso );
                        miseAJourSprite( &Ennemie);
			SDL_FillRect(ecran, NULL, 0);
			Afficher_Perso( &Perso, ecran );
                        Afficher_Perso( &Ennemie, ecran );	
